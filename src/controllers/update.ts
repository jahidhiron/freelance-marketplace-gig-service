...
import { BadRequestError, ISellerGig, isDataURL, uploads } from '@jahidhiron/jobber-shared';
...
export const gigUpdateActive = async (req: Request, res: Response): Promise<void> => {
  const updatedGig = await updateActiveGigProp(req.params.gigId, req.body.active);
  res.status(StatusCodes.OK).json({ message: 'Gig updated successfully.', gig: updatedGig });
};
